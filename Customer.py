...
from DataLoaders import CustomerSide
import math
...
class RestaurantApp:
    def __init__(self, root: tk.Tk):
        ...
        self.qtyvars = [{'Name':a['Name'], "QtyVar": tk.IntVar(root, 0), 'Qty': 0} for a in self.menu_items_linear]
        self.order = []
    ...
    def create_header(self):
        ...
        restaurant_name = ttk.Label(header_frame, text="Your Restaurant Name", font=("Arial", 16, "bold"))
    ...
    def create_checkout_button(self):
        ...
        def checkout_button_click_animation():
            ...
            def on_release(event):
                canvas.itemconfig(button, fill="blue")  # Change back to original color on release
    ...
    def update_order(self):
        for a in self.qtyvars:
            a['Qty'] = a["QtyVar"].get()
        print([a for a in self.qtyvars if a['QtyVar'].get()>0])
