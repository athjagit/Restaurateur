...
class LoginWindow:
        ...
        if mode == 1: 
                ...
                with open('users.csv', newline='') as file:
                    ...
                    for row in reader:
                        if row['username'] == username and row['password'] == password:
                            self.app = RestaurantApp(self.parent, username=username)
            ...
            try:
                    ...
                    if 'a' == username and 'b' == password:
                        self.app = AdminDashboard(self.parent)
                        self.app.user = username
                        self.top.destroy()
                        return
                    messagebox.showerror("Login Failed", "Invalid username or password.")
    ...
    def create_login_button(self):
        ...
        login_customer = tk.Button(button_frame, text="Customer", font=("Arial", 12, "bold"), 
                                 ...
                                 activebackground="#45A049", height=2, command=lambda: self.login(mode=1))
        ...
        login_restaurant = tk.Button(button_frame, text="Restaurant",  font=("Arial", 12, "bold"), 
                                 bg="#4CAF50", fg="white", bd=0, relief="flat", cursor="hand2", padx = 3,
                                 activebackground="#45A049", height=2, command=lambda: self.login(mode=0))
    ...
    def clear_focus(self, event):
        widget = event.widget
        if self.top.winfo_exists() and not isinstance(widget, tk.Entry):
            self.top.focus_set()
